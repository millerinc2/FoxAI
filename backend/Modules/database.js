...
// Usamos la carpeta 'Session' según tu estructura definida
const DATA_DIR = path.join(__dirname, '../Session');
...
const JSONEngine = {
    ...
    write: async (key, data) => {
        if (dbState.locks.has(key)) return; 
...
const ChatDB = {
    saveMessage: async (userId, role, content) => {
        const messageData = { userId, role, content, timestamp: new Date() };
    ...
    getChatHistory: async (userId, limit = 50) => {
        ...
        const localChats = JSONEngine.read('chats');
        return localChats.filter(c => c.userId === userId).slice(-limit).reverse();
    }
};
