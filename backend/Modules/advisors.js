...
class AdvisorSystem {
    ...
    async assignAdvisor(userId) {
        try {
            ...
            return ADVISOR_POOL[0];
        }
    }

    buildAdvisorPrompt(advisor, userProfile) {
        return `ERES UN HUMANO REAL TRABAJANDO COMO ASESOR TÉCNICO EN FOXAI.
TU IDENTIDAD:
Nombre: ${advisor.name}
...
- Mantén siempre el profesionalismo según tu personalidad.
- Si eres de ${advisor.country}, usa expresiones naturales.
- Estás atendiendo a ${userProfile.name || 'un usuario'}.`;
    }
    ...
    async generateAdvisorResponse(userId, message) {
        ...
        return {
            ...
            text: `(Simulación) Hola ${memory.profile?.name || 'amigo'}, entiendo tu duda.`,
    ...
    getInitialMenu() {
        return `Hola, bienvenido al Soporte de FoxAI 🦊\nPor favor, selecciona una opción:\n🛠️ Soporte Técnico\n💳 Membresías y Pagos\n📑 Reclamos y Sugerencias\n👤 Hablar con un asesor humano`;
    }
}
